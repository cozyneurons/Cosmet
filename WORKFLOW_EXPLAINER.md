# Cosmix Multi-Agent Safety Analysis Workflow Explainer

This document provides a detailed technical breakdown of how the **Cosmix** cosmetic safety analysis application works under the hood.

---

## 1. High-Level Architecture Overview

Cosmix is designed as a modular web application split into three main layers:
...
* **Nodes:** Each agent or utility represents a node in the graph. In Cosmix, the nodes are:
...
* **Edges:** Connections between nodes. Cosmix uses **conditional edges** originating from the `supervisor` to determine which node to activate next, depending on the current graph state.

### Code Directories and Key Files

All LangGraph and Agentic logic is located under the backend services:

* **Graph State definition:** [state.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/graph/state.py)
...
* **Graph Workflow & Compilation:** [workflow.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/graph/workflow.py)
...
* **Supervisor Agent Node:** [supervisor.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/agents/supervisor.py)
  Decides transitions based on the current state.
* **Research Agent Node:** [research_agent.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/agents/research_agent.py)
  Runs database queries and fallback Tavily web searches.
* **Analysis Agent Node:** [analysis_agent.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/agents/analysis_agent.py)
  Generates reports dynamically via Gemini 3.1 Flash Lite.
* **Critic Agent Node:** [critic_agent.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/services/agents/critic_agent.py)
...
2. **Server-Side Rendering (SSR) Hydration Fix:**
   * **Layout Auth Syncing:** Added a `mounted` state inside `DashboardLayout` in [layout.tsx](file:///Users/shubhamkumar/Desktop/Cosmix%202/frontend/src/app/(dashboard)/layout.tsx) so client-only cookie checks wait until after the initial render. This prevents the server (rendering `null`) and client (rendering the dashboard) from failing React hydration.
   * **Root Tag Warning Suppression:** Added `suppressHydrationWarning` to the `<html>` element in the root [layout.tsx](file:///Users/shubhamkumar/Desktop/Cosmix%202/frontend/src/app/layout.tsx) to prevent browser extensions and testing tools from triggering console errors.

### Backend
1. **Environment Variables Loading:** Added `load_dotenv()` to [config.py](file:///Users/shubhamkumar/Desktop/Cosmix%202/backend/app/core/config.py) so variables inside `backend/.env` are correctly loaded into `os.environ` upon startup. This resolved a crash where agents threw `GOOGLE_API_KEY not found in environment`.
